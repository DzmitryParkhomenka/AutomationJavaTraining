...
public class ReportUXThirdTable {
	public static void ReportUXThird(Connection con, Connection con2, ResultSet rs, ResultSet rs2, Statement st,
			Statement st2) throws ClassNotFoundException, SQLException {
		// create object of Compare class
		Compare compare = new Compare();

		// create result set objects
		rs = st.executeQuery("SELECT COUNT(*) FROM Cloud.dbo.Report_Group_Report WHERE PCN = " + compare.getPlexusCustomerNumberSource());
		rs2 = st2.executeQuery("SELECT COUNT(*) FROM Cloud.dbo.Report_Group_Report WHERE PCN = " + compare.getPlexusCustomerNumberTarget());
